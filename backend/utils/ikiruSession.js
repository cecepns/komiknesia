 ...
 * Dipakai oleh IkiruSyncController (termasuk POST /api/ikiru/cron-sync & /api/admin/ikiru-sync/cron-sync) dan IkiruScrapController.
 */
...
async function performIkiruLogin() {
  ...
  if (!loginUrl) {
    if (!authFormStillPresent(authRes.data)) {
      throw new Error(
  ...
  if (authFormStillPresent(verifyRes.data)) {
