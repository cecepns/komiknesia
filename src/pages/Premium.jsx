...
const Premium = () => {
  return (
    <div className="min-h-screen bg-gray-950 text-white pt-5 md:pt-20 pb-4">
      <Helmet>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl border border-amber-400/20 bg-gradient-to-b from-amber-500/10 via-gray-900 to-gray-950 p-6 md:p-10">
          <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top,rgba(251,191,36,0.18),transparent_50%)]" />

          <div className="relative text-center">
            <span className="inline-flex items-center gap-2 rounded-full border border-amber-300/30 bg-amber-400/10 px-4 py-1.5 text-xs font-semibold tracking-[0.2em] text-amber-300 uppercase">
              <Sparkles className="h-3.5 w-3.5" />
              Premium Membership
            </span>

            <h1 className="mt-5 text-4xl md:text-6xl font-extrabold leading-tight">
              Upgrade ke <span className="text-amber-400">VIP Premium</span>
            </h1>
            ...
            {premiumPackages.map((pkg) => (
              <article
                ...
                className={`rounded-2xl border p-5 md:p-6 shadow-lg ${
                  pkg.highlight
                    ? "border-amber-400 bg-gradient-to-b from-amber-500/20 to-gray-900 ring-1 ring-amber-300/50"
                    : "border-white/15 bg-white/[0.03]"
                }`}
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <p className="text-xs uppercase tracking-[0.16em] text-gray-400">{pkg.name}</p>
                    <p className="mt-3 text-4xl font-black">{pkg.price}</p>
                    <p className="text-sm text-gray-400">untuk {pkg.duration}</p>
                  </div>
                  {pkg.highlight && (
                ...
                <div className="mt-6 border-t border-white/10 pt-4 space-y-2.5">
                  {benefits.map((benefit) => (
                    <div key={benefit} className="flex items-center gap-2 text-sm text-gray-200">
